...
class CasaController {
    ...
    async excluirCasaPorId(req, res) {
        ...
        if (!id_casa || !id_user) {
            return res.status(400).json({
                mensagem:
                    'dados inválidos. Deve ser informado o id da casa e o id do usuário',
            });
        }
    ...
    async buscarCasas(req, res) {
        try {
            ...
            const dadosBusca = UtilShared.retirarDadosNulosDeObjeto({
                situacao: situacao,
                idDono: id_dono,
            });
                ...
                casasArray = casasArray
                    .map((el) => {
                        ...
                        if (
                            ...
                            return preco >= preco_min && preco <= preco_max
                                ? el
                                : null;
                        }
            ...
            return res.json({
                mensagem: `Erro ao tentar obter casas ${error}`,
            });
        }
    }
    ...
    async buscarCasasV2(req, res) {
        try {
            const { situacao, id_dono, preco_max, preco_min } = req.query;
            const filtro = UtilShared.retirarDadosNulosDeObjeto({
                situacao: situacao,
                idDono: id_dono,
            });
    ...
    async criarCasa(req, res) {
        try {
            ...
            if (!ehValidoSchema) {
                return res
                    .status(400)
                    .json({ msg: 'ERRO: Falha na validação.' });
            }

            // Verifica se já existe uma casa com este mesmo endereço ou descrição
            const jaExisteCasaDescricao = await this.searchBy(
                'descricao',
                descricao,
                'Casa',
            );
            const jaExisteCasaEndereco = await this.searchBy(
                'descricao',
                descricao,
                'Casa',
            );
            ...
            res.status(500).json({
                ERRO: `Não foi possível processar a requisição ${error}`,
            });
        }
    }
