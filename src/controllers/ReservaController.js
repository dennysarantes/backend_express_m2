...
class ReservaController {
    ...
    async cancelarReservaPorCodigo(req, res){
        ...
        if(!codigoReserva || !id_usuario){
            return res.status(401).json({mensagem: "dados inválidos. Deve ser informado o id da reserva e o id do usuário"});
        }
        ...
        if(String(usuario?._id) !== String(reservaReferencia?.responsavel_id)){
        ...
        try {
            ...
            if(reservaEditada){
                return res.status(200).json({msg: 'Reserva cancelada com sucesso.'});
            }else{
    ...
    async excluirReservaPorCodigo(req, res){
        ...
        try {
            const reservaDeletada = await Reserva.findByIdAndDelete(reservaReferencia._id);
    ...
    async listarReservas(req, res){
         let reservas = []
         const { id_usuario, id_casa, paginaIndex, qtdElementosPorPagina  } = req.body;
 
        const limit = !qtdElementosPorPagina ? 4 : qtdElementosPorPagina;
        const skip = ((!paginaIndex ? 1 : paginaIndex) - 1) * qtdElementosPorPagina;
         ...
         try {
            ...
            reservas = await Reserva.find(objBusca)
                 .skip(skip)
                 .limit(limit)
            ...
            console.error(error);
             return res.status(500).json({msg: 'ERRRO ao tentar listar as reservas. Tente novamente mais tarde.'})
         }
     }
    ...
    async nova(req, res) {
        ...
        try {
           const responsavelValido = await this.ehUsuarioValido(responsavel_id); 
           if(!responsavelValido){
                return res.status(401).json({msg: 'Id de responsável inválido'});
           }
        } catch (error) {
            return res.status(400).json({msg: error ?? 'Responsável inválido'});
        };
        ...
        try {
            ...
            if(!dataInicioReserva || !dataFimReserva){
                return res.status(400).json({msg: 'Data dataInicioReserva e dataFimReserva são obrigatórios'});
            }
            
            //Verifica se as datas são válidas
            const datasReserva = UtilShared.validarDatasReserva(dataInicioReserva, dataFimReserva);
            ...
            const reservaExistente = await this.ehReservaExistente(casa_id, dataInicioReserva,dataFimReserva);
            console.log("reservaExistente: ", reservaExistente);

            if (reservaExistente) {
                return res.status(400).json({msg: 'A casa já está reservada para as datas fornecidas'});
            }else{
                const codigoReserva = UtilShared.gerarCodigoHash(responsavel_id.toString());

               const reserva = await Reserva.create({
                    dataSolicitacao: new Date(),
                    dataInicioReserva: UtilShared.dataBrParaDate(dataInicioReserva),
                ...
                return reserva ?
                         res.status(200).json({msg: 'Reserva criada com sucesso', codigoReserva: reserva.codigoReserva}) :
                         res.status(500).json({msg: 'ERRO: Algo deu errado. Tente novamente mais tarde.'});
            }
    ...
    async ehReservaExistente(casa_id, dataInicioReserva, dataFimReserva){
        ...
        console.log('Datas normalizadas para a nova reserva (sem horas):', dataInicio, dataFim);
        ...
        const reservasRelevantes = await Reserva.find({
            ...
            $or: [
                ...
                { dataInicioReserva: { $lte: dataInicio }, dataFimReserva: { $gte: dataFim } } // Reservas que cobrem todo o intervalo
            ]
        });
        ...
        for (const reserva of reservasRelevantes) {
            ...
            if (
                (dataInicio < reservaFim && dataFim > reservaInicio) // Conflito se a nova reserva inicia antes do fim e termina após o início da reserva existente
            ) {
                console.log('Conflito de reserva detectado com a reserva:', reserva);
